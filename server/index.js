...
app.post("/signup", (req, res) => {
    ...
    usersCollectionRef
        ...
        if (searchResponse.empty) {
            ...
            res.status(400).json({
                message: "El email ingresado ya fue usado, por favor ingrese otro (Me salió una rima)",
            });
        }
    });
...
app.post("/createRoom", (req, res) => {
    ...
    const roomRef = db_1.realtimeDB.ref("rooms/" + (0, uuid_1.v4)());
    console.log(userId);
    usersCollectionRef
        .doc(userId.toString())
        .get()
...
app.patch("/gameRoom/:roomId/:userId", (req, res) => {
    ...
    const userStatus = req.body.userStatus; // true o false
    const userName = req.body.userName; // nombre del usuario
    const roomRef = db_1.realtimeDB.ref("/rooms/" + roomId);
...
app.patch("/joinRoom/:roomId/:userId", (req, res) => {
    ...
    roomRef.get().then((currentGameSnap) => {
        ...
        if (message === "Te has unido a la sala!" ||
